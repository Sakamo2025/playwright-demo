...
test('グループ一覧確認', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'メールアドレス' }).fill('keiichi.sakamoto+pwgroup0202@medimo.ai');
  ...
  await page.getByRole('textbox', { name: 'メールアドレス' }).fill('keiichi.sakamoto+pwgroup02@medimo.ai');
  await page.getByRole('button', { name: '更新' }).click();
  ...
  await page.getByRole('button', { name: 'プランを選択' }).click();
  await page.getByRole('button', { name: '35 テスト用_Standard 49,500' }).click();
